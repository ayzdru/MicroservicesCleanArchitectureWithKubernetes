...
{   
    public class ProductServiceV1 : Product.ProductBase
    {
        ...
        public override async Task<ProductsResponse> GetProducts(Empty request, ServerCallContext context)
        {
            ...
            productsResponse.Products.AddRange(products.Select(q=> new ProductResponse() { Id = q.Id.ToString(), Name = q.Name, Description = q.Description, Price = q.Price, PriceCurrency = q.PriceCurrency }));
        ...
        public override async Task<ProductResponse> GetProductById(GetProductByIdRequest request, ServerCallContext context)
        {
            ...
            return await Task.FromResult( new ProductResponse() { Id = product.Id.ToString(), Name = product.Name, Description = product.Description, Price = product.Price, PriceCurrency = product.PriceCurrency });
        }
    }
}
